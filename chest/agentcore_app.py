...
def _run_sweep(session_id: str) -> str:
    ...
    identity_key = f"agentcore:{session_id}"
    account = AccountStore().account_for(identity_key)
    if account is None:
        return "Link this AgentCore session to an organization before running a sweep."
    ...
    if not message.startswith("BLOCKED:"):
        DraftStore().stage(f"{account.id}:{identity_key}", message)
    return message
