...
class FakeDrafts:
    def __init__(self):
        self.pending_sessions = {"acct_test:chat-1"}
        self.approved = []
        self.edits = []
...
def test_yes_approves_the_pending_grant_draft_without_calling_the_model(monkeypatch):
    ...
    monkeypatch.setattr(
        webhook.ACCOUNTS,
        "account_for",
        lambda _: Account(name="Test Org", id="acct_test"),
    )
    monkeypatch.setattr(
        webhook, "for_session", lambda *_: (_ for _ in ()).throw(AssertionError())
    )

    reply = webhook.handle("chat-1", "YES")

    assert drafts.approved == ["acct_test:chat-1"]
...
def test_edit_records_instructions_for_the_pending_draft(monkeypatch):
    ...
    assert drafts.edits == [("acct_test:chat-1", "make the opening shorter")]
